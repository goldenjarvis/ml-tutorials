...
## Functional toolbox structure

The codebase is organized as a pure-function toolbox grouped by domain:

```txt
src/
  index.js                # MLToolbox public API
  fp/                     # composition helpers: pipe/compose/mapWith/reduceWith
  data/                   # sample generation utilities
  stats/                  # mean/variance/std/min-max/normalizers
  calculus/               # slope, derivative, higher-order derivatives
  transforms/
    linear.js             # lerp, affine, range mapping, shift, scale
    warps.js              # log/sigmoid/tanh/power/reciprocal warps
dist/
  mltb.js                 # single-file CDN/browser build (global: MLToolbox)
  mltb.min.js             # minified CDN build
playground/
  index.html              # drag-drop graph editor
  playground.js           # editor logic and runtime integration
  styles.css
index.js                  # runnable Node demo
scripts/
  build.js                # esbuild pipeline (source of dist files)
```
...
ML.fp.pipe(...fns);
ML.data.randomSamples({ count: 32, offset: 0, span: 1 });
ML.stats.mean(values);
ML.calculus.derivative(values);
ML.transforms.linear.createRangeMapper({ inMin: 0, inMax: 1, outMin: -1, outMax: 1 });
ML.transforms.warps.createSigmoidWarp();
ML.graph.executeGraph({ graph });
```

## Neural Network Graph Runtime (MVP)

You can build a model as a graph of reusable nodes:

- `data.generator.linear`
- `model.weightInit.linear`
- `model.hypothesis.linear`
- `training.loss.mse`
- `training.gradient.linearMSE`
- `training.update.sgd`
- `control.forLoop`
- `observe.metric.collector`

### Key concept: delayed edges

Training feedback (`updated weights -> hypothesis weights`) must be marked as delayed edges:

```js
{ from: { node: 'update', port: 'w' }, to: { node: 'hyp', port: 'w' }, delay: true }
```

`delay: true` means \"use previous loop step value\", which avoids same-step cycles.
...
This runs a full training graph and prints initial/final metrics.

### Visual drag-drop tool
...
```
3. Open:
`http://localhost:4173/playground/index.html`
4. Click `Load Example` and `Run Graph`.
5. Drag nodes from palette, connect ports, set params/scopes, then run again.
...
Do not edit files in `dist/` directly. They are generated by `scripts/build.js`.

Then host `dist/mltb.min.js` (or `dist/mltb.js`) on any static host (GitHub Pages, S3, Cloudflare R2, etc.) and use:

```html
<script src="https://your-cdn.example.com/mltb.min.js"></script>
<script>
  const samples = MLToolbox.data.randomSamples({ count: 10, offset: 5, span: 2 });
  const mean = MLToolbox.stats.mean(samples);
  console.log(mean);
</script>
```

For GitHub-based CDN patterns, publish tagged releases and point jsDelivr/unpkg to `dist/mltb.min.js`.
